...
public class BookProvider extends ContentProvider {
    private BookStoreHelper mDBHelper;
    ...
    public boolean onCreate() {
        mDBHelper = new BookStoreHelper(getContext());
        return true;
    }
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // If the {@link BookContract.BookEntry.COLUMN_NAME} key is present,
        ...
        // If the {@link BookContract.BookEntry.COLUMN_$} key is present,
        ...
        // If the {@link BookContract.BookEntry.COLUMN_QUANTITY} key is present,
        // check that the weight value is valid.
        if (values.containsKey(BookContract.BookEntry.COLUMN_QUANTITY)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer quantity = values.getAsInteger(BookContract.BookEntry.COLUMN_QUANTITY);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException("Book requires qunatity");
            }
        }

        // If the {@link BookContract.BookEntry.COLUMN_SUPPLIER} key is present,
        // check that the weight value is valid.
